...
function MindVaultPage() {
  ...
  const analytics = overview?.analytics || { subjects: [], forecast: [] };
  ...
  const submitSubject = async (event) => {
    ...
    try {
      const payload = {
        ...subjectForm,
        tags: normalizeTagsInput(subjectForm.tags),
  ...
  const submitItem = async (event) => {
    ...
    try {
      const payload = {
        ...
        tags: normalizeTagsInput(itemForm.tags),
    ...
    <section className="page mindvault-page">
      <div className="page-header">
        <div>
          ...
          <p className="lead">
            Plan what to learn, break it into sprint-sized chunks, and keep today&apos;s reminder queue small enough to actually finish.
          </p>
        </div>
        <div className="mindvault-actions">
          <button className="button secondary" onClick={quickAddItem}>
            Quick add topic
          </button>
          <button className="button secondary" onClick={() => setActiveForm('subject')}>
            New subject
          </button>
          <button className="button secondary" onClick={() => setActiveForm('sprint')}>
            New sprint
          </button>
        </div>
      </div>
        ...
        <div className="mindvault-column">
          <section className="panel">
            <div className="panel-header">
              <div>
                <p className="eyebrow">Capture + plan</p>
                <h3>{activeForm === 'subject' ? 'Subject planner' : activeForm === 'sprint' ? 'Sprint planner' : 'Topic capture'}</h3>
              </div>
            ...
            <div className="segmented compact">
              <button className={activeForm === 'item' ? 'active' : ''} onClick={() => setActiveForm('item')}>
                Topic
              </button>
              <button className={activeForm === 'subject' ? 'active' : ''} onClick={() => setActiveForm('subject')}>
                Subject
              </button>
              <button className={activeForm === 'sprint' ? 'active' : ''} onClick={() => setActiveForm('sprint')}>
                Sprint
              </button>
            </div>
            ...
            {activeForm === 'sprint' ? (
              <form className="mindvault-form" onSubmit={submitSprint}>
                ...
                <div className="form-actions">
                  ...
                  <button className="button secondary" type="button" onClick={() => setSprintForm({ ...sprintDefaults, subjectId: subjectFilter !== 'all' ? subjectFilter : '' })}>
                    Clear
                  </button>
                </div>
              </form>
              ...
              {filteredItems.map((item) => (
                <article key={item.id} className="entity-card">
                  ...
                  <div className="muted-row">
                    ...
                    <span>Interval {item.reviewIntervalDays || 1} day(s)</span>
                    <span>{item.nextReviewDate ? `Next ${formatDate(item.nextReviewDate)}` : 'No next review'}</span>
                  </div>
                </article>
              ))}
            </div>
            ...
            {selectedQueueItem ? (
              <div className="study-card">
                ...
                <label className="field">
                  ...
                  <textarea className="input textarea" rows="2" value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} placeholder="What was easy, what was hard, what to fix next time?" />
                </label>
                <div className="rating-row">
                  {reviewRatings.map((rating) => (
                    <button type="button" key={rating.value} className={`rating-button rating-${rating.value}`} disabled={busy} onClick={() => submitReview(rating.value)}>
...
function sortItems(left, right, sort) {
  switch (sort) {
    ...
    case 'due':
    default:
      return String(left.nextReviewDate || left.dueDate || '').localeCompare(String(right.nextReviewDate || right.dueDate || ''));
  }
}

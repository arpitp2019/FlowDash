...
test('MindVault supports subject planning, sprint capture, queue review, and spaced repetition', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /Learning management with subjects, sprints, and spaced review/i })).toBeVisible();
  ...
  await page.getByLabel('Subject').selectOption({ label: 'Physics' });
  ...
  await expect(page.getByText('Wave foundations')).toBeVisible();
  ...
  await expect(page.getByRole('button', { name: /Quantum basics/i })).toBeVisible();

  await page.getByRole('button', { name: /Quantum basics/i }).click();
